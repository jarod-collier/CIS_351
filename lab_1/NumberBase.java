...
/*
* Authors: Jarod Collier and Josef Hartsough
*/
...
public class NumberBase {
	...
	private static int toBase10(String input, int base_in) {
        ...
        for (; i < end_of_word; i++) {
            n--;
            int digit = constants_to_int.get(input.charAt(i));
            sum += digit * Math.pow(base_in, n);
        }
	...
	private static String toBaseN(int input, int base_out) {
        ...
        while (n > 0) {
            d = n / base_out;
            int remainder = n % base_out;
            sum = constants_from_int.get(remainder) + sum;
            n = d;
        }
